...
class ProsesPasien implements KontrakPresenter
{
	...
	function __construct()
	{
		...
		try {
			...
			$db_name = "dpbo_lp12"; // nama basis data
	...
	function prosesDataPasien()
	{
		try {
			...
			// delete checker
			if (isset($_GET['delete'])) $this->tabelpasien->delete($_GET['delete']);

			// submit checker
			if (isset($_POST['submit'])) {
				if (isset($_GET['update'])) $this->tabelpasien->update($_GET['update']);
				else $this->tabelpasien->add();
			}
	...
	{
		$this->tabelpasien->open();
		$this->tabelpasien->getDetailPasien($id);
